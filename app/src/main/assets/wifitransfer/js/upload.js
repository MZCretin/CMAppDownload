...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            clearData()
            data.forEach(item => {
                tbodyAppendTr("table_body", 0, item.name, item.size, "上传成功", item.time)
            })
        },
        error: function() {
            //console.log("getDeviceInfo failed...");  
...
function tbodyAppendTr(tbodyid, rowIndex, col1, col2, col3, col4) {
    $("#" + tbodyid).append("<tr><td>" + col1 + "</td><td>" + col2 + "</td><td>" + col3 + "</td><td>" + col4 + "</td></tr>");
    if (isIEBelow(10)) {
...
function addTr(tbody, row, fileName, fileSize) {
    if (!equals(fileName, fileSize)) {
        ...
        // } else {
        tbodyAppendTr("table_body", filledRowIndex, fileName, fileSize, CONSTANTS.UPLOAD_WAIT, getCuttentTime());
        // }
        filledRowIndex++;
    } else {
